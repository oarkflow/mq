...
func (d *DAG) respond(task mq.Result) mq.Result {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *DAG) processTaskResult(queue, taskID string, totalItems int, multipleResults bool) {
	d.taskResults[taskID] = map[string]*taskContext{
		queue: {
