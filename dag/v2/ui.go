...
func (tm *DAG) ExportDOT() string {
	...
	for _, nodeKey := range sortedNodes {
		...
		nodeColor := "lightblue"
		sb.WriteString(fmt.Sprintf(`  "%s" [label=" %s", fillcolor="%s", id="node_%s"];`, node.ID, node.Label, nodeColor, node.ID))
		sb.WriteString("\n")
	}
	for _, nodeKey := range sortedNodes {
		node, _ := tm.nodes.Get(nodeKey)
		for _, edge := range node.Edges {
			...
			edgeColor := "black"
			sb.WriteString(fmt.Sprintf(`  "%s" -> "%s" [label=" %s", color="%s", style=%s, fontsize=10, arrowsize=0.6];`, node.ID, edge.To.ID, edge.Label, edgeColor, edgeStyle))
			sb.WriteString("\n")
		}
	}
		...
		if node.processor != nil {
			...
			if subDAG != nil {
				...
				subDAG.nodes.ForEach(func(subNodeKey string, subNode *Node) bool {
					sb.WriteString(fmt.Sprintf(`    "%s" [label=" %s"];`, subNodeKey, subNode.Label))
					sb.WriteString("\n")
					return true
				})
				subDAG.nodes.ForEach(func(subNodeKey string, subNode *Node) bool {
					for _, edge := range subNode.Edges {
						sb.WriteString(fmt.Sprintf(`    "%s" -> "%s" [label=" %s", color="black", style=solid, arrowsize=0.6];`, subNodeKey, edge.To.ID, edge.Label))
						sb.WriteString("\n")
					}
					return true
				})
				sb.WriteString(`  }`)
				sb.WriteString("\n")
				sb.WriteString(fmt.Sprintf(`  "%s" -> "%s" [label=" %s", color="black", style=solid, arrowsize=0.6];`, node.ID, subDAG.startNode, subDAG.name))
				sb.WriteString("\n")
			}
		}
	}
