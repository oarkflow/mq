...
func main() {
	...
	checkValidityNode := &FunctionNode{
		...
		Func: func(task *Task) Result {
			...
			if !isValidEmail(email) || !isValidPhone(phone) {
				return Result{
					NextNodeID: "customRegistration",
					Message:    "Invalid email or phone number. Please try again.",
				}
			}
			return Result{NextNodeID: "checkManualVerification"}
		},
	}
	...
	checkManualVerificationNode := &FunctionNode{
		...
		Func: func(task *Task) Result {
			...
			if city != "Kathmandu" {
				return Result{NextNodeID: "manualVerificationPage"}
			}
			return Result{NextNodeID: "approveCustomer"}
		},
	}
	...
	approveCustomerNode := &FunctionNode{
		...
		Func: func(task *Task) Result {
			...
			return Result{NextNodeID: "sendVerificationEmail"}
		},
	}
	...
	sendVerificationEmailNode := &FunctionNode{
		...
		Func: func(task *Task) Result {
			...
			return Result{NextNodeID: "verificationLinkPage"}
		},
	}
